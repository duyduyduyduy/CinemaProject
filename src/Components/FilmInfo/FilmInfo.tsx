...
function FilmInfo(props: any) {
 ...
 useEffect(() => {
  ...
  console.log(props.CurrentFilmState);
   let ObjectFilmInfo=props.CurrentFilmState.lsCurFilm.filter((n:eFilm) => n.id===id) 
    if(!ObjectFilmInfo[0])
    {
      ObjectFilmInfo=props.NextFilmState.lsNextFilm.filter((n:eFilm) => n.id===id) 
    }
    if(ObjectFilmInfo[0])
    {
  ...
  return (
    <div style={{ marginBottom: "100px" }}>
      ...
      <div className="bannerContainer" id="FilmBanner"
        style={{
      ...
      >
        <div className="overlapbanner">
          ...
          <div>
            ...
            <h3>{FilmInfo[0]?.name} · 2002 · {FilmInfo[0]?.duration} phút</h3>

            <h2 className="nd-h2">Nội dung</h2>

            <p>Lấy bối cảnh sau hơn một thập kỷ kể từ phần phim đầu tiên, Avatar: Dòng Chảy Của Nước kể về câu 
              chuyện của gia đình Sully (Jake, Neytiri, và con của họ), những vấn đề xung quanh họ, những nỗ 
              lực để bảo vệ sự an toàn của gia đình, những cuộc chiến sinh tồn, và những hiểm họa mà họ phải
               đối mặt.
            </p>
            ...
            <div className="infoFilm">
              <p>
                ...
                <span>{FilmInfo[0]?.startdate.replace("T00:00:00.000Z", "")}</span>
              </p>
